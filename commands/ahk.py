...
def execute(parent, commands, user, host, channel, params):
	...
	if len( matches ) > 0:	
		return {'Status': 0,'Text': "\x02" + matches[0] + "\x02 - " + parent.shorten_url(docdata[matches[0]]), 'Error': 'No error'}

	try:
		if len( params ) == 0:
			return {'Status': -1, 'Text': 'Usage: &botcmdahk <query>', 'Error': 'Command needs more params'}
		...
		return {'Status': -1, 'Text': 'No data found!', 'Error': 'Index Error '}
	return {'Status': 0, 'Text': "\x02" + urllib.parse.unquote( title ) + "\x02 - " + parent.shorten_url(url), 'Error': 'No error'}
